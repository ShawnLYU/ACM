...
int main() {
	...
	while (cin >> n) {
		...
		for (int i = 0; i<m; i++) {
			...
			/*
			if (isNumber(remaining[t - 3])) {//two positive integers
			for (int i = 0; i<t - 4; i++)
			destination += (remaining[i] + " ");
			destination += remaining[t - 4];
			from = stoi(remaining[t - 3]);
			to = stoi(remaining[t - 2]);
			//cout << "From " << source << " to " << destination << " is " << from << endl;
			//cout << "From " << destination << " to " << source << " is " << to << endl;
			isSingle = false;
			}
			else {//one positive integer
			for (int j = 0; j<t - 3; j++)
			destination += (remaining[j] + " ");
			destination += remaining[t - 3];
			from = stoi(remaining[t - 2]);
			//cout << "From " << source << " to " << destination << " is " << from << endl;
			isSingle = true;
			}
